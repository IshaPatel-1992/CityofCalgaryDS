...
export const crimeDataGroupedByYear = async (req, res) => {
    console.log(">>>>>crimeDataGroupedByYear:"+JSON.stringify(req.query.community));
...
export const crimeDataGroupedByCrimeTypePieChart = async (req, res) => {
  console.log(">>>>>crimeDataGroupedByCrimeType for Pie Chart:"+req.query.community+"====" +req.query.year);
const community = req.query.community;
...
const year = req.query.year;
try {
  const dataByMonth = await getDataGroupedByCrimeTypeAndMonth(community,year);
